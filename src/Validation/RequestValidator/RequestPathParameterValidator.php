 ...
 */
final class RequestPathParameterValidator implements ValidatorInterface
{
    /**
     * @var Validator
     */
    ...
    public function validate(Request $request): ?RequestProblemExceptionInterface
    {
        ...
        if (count($violations) > 0) {
            $exception = new InvalidRequestParameterProblemException(
                ...
                'Validation of path parameters failed.'
            );
        ...
        $validationContext[ValidationContext::REQUEST_PATH_PARAMETERS] =
            \json_encode($this->getValidatedPathParametersWithValues($validatePathParameters, $request));
